...
export default function IctSupportRegisterDetail({ reportId, onBack, onEdit }: Props) {
  ...
  const issueOpts = line.support_category === "helpdesk" ? ICT_HELPDESK_ISSUES : ICT_NETWORK_ISSUES;
        ...
        <div className="w-full px-4 md:px-6 py-4 space-y-4 pb-8">
          ...
          <SectionCard title="Support Details">
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
              <Field label="Support ID" value={line.support_id} />
              <Field label="Date Reported" value={formatDate(line.date_reported)} />
              <Field label="Reported By" value={line.reported_by} />
              <Field label="Support Category" value={labelOf(ICT_SUPPORT_CATEGORIES, line.support_category)} />
              <Field label="Issue Type" value={labelOf(issueOpts, line.issue_type)} />
              <Field
                label="Priority"
                value={line.priority ? (
                  <Badge className={`text-[10px] border ${PRIORITY_BADGE[line.priority] ?? ""}`}>
                    {labelOf(ICT_PRIORITIES, line.priority)}
                  </Badge>
                ) : null}
              />
              <Field label="Description of Issue" value={line.description} full />
            </div>
          </SectionCard>

          <SectionCard title="Resolution & Closure">
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
              <Field label="Date Resolved" value={formatDate(line.date_resolved)} />
              <Field
                label="Status"
                value={line.resolution_status ? (
                  <Badge className={`text-[10px] border ${STATUS_BADGE[line.resolution_status] ?? ""}`}>
                    {labelOf(ICT_RESOLUTION_STATUSES, line.resolution_status)}
                  </Badge>
                ) : null}
              />
              <Field label="External Support Required" value={labelOf(ICT_YES_NO, line.external_support_required)} />
              <Field label="Referred To" value={labelOf(ICT_REFERRED_TO, line.referred_to)} />
              <Field label="Action Taken" value={line.action_taken} full />
